...
local function send_json(res, status_code, data)
    ...
    if not body then
        body = '{"error":"Unable to encode server response"}'
        status_code = 500
    end
...
local function handle_message(req, res)
    read_request_body(req, function(body, body_error)
        ...
        -- IMPORTANT:
        -- Do not finish the HTTP response here.
        --
        -- Ollama is asynchronous. Alice will call this callback when
        -- generation has completed or an error occurs.
